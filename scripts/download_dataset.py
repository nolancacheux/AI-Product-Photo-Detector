...
def save_images(
    ...
    """Save images as JPEG files, resizing if needed."""
    output_dir.mkdir(parents=True, exist_ok=True)
    count = 0

    for i, img in enumerate(images):
        img_rgb = img.convert("RGB")
        if img_rgb.size != (target_size, target_size):
            img_rgb = img_rgb.resize((target_size, target_size), Image.LANCZOS)
        img_rgb.save(output_dir / f"{prefix}_{i:05d}.jpg", "JPEG", quality=92)
        count += 1
...
def main() -> None:
    ...
    for label_id, label_name in LABEL_MAP.items():
        ...
        for split_name, split_images_list in splits.items():
            ...
            count = save_images(split_images_list, dest, label_name, args.target_size)
    ...
    print(f"\nImage size: {args.target_size}x{args.target_size} JPG")
